...
### Task 13: YouTube 구독 목록 OAuth 동기화 (보류 — 2026-08-31 범위 조정)

> **보류 사유**: 실제로 검증된 수요는 "게임 개발 채널 요약 → 다른 게임 프로젝트가 참고"(수동 `/addchannel` + 검색)였다. 구독 목록 자동 동기화는 아직 아무도 안 쓴 기능을 위해 계속 투자하는 가정이라, 검증된 방향(수동 채널 등록 + 검색 커맨드)에 먼저 집중하기로 했다. 필요해지면 이 태스크를 다시 꺼낸다 — 코드/설계는 아래 그대로 유효하다.
